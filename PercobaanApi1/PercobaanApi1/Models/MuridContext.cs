...
namespace PercobaanApi1.Models
{
    public class MuridContext
    {
        ...
        public List<Murid> ListMurid()
        {
            ...
            string query = @"SELECT id_murid, nama, kelas, alamat, email FROM users.murid;";
        ...
        {
            Murid person = null;
            string query = $"SELECT id_murid, nama, kelas, alamat, email FROM users.murid WHERE id_murid = {id_murid};";
            SqlDBHelper db = new SqlDBHelper(this.__constr);
            {
                try
                {
        ...
        {
            string query = $@"INSERT INTO users.murid (id_murid, nama, kelas, alamat, email) 
        ...
        public void UpdateMurid(Murid person)
        {
            string query = $@"UPDATE users.murid 
                              SET nama = '{person.nama}', kelas = '{person.kelas}', alamat = '{person.alamat}', email = '{person.email}' 
